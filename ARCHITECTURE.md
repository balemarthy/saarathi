...
- `search_web(query: string)` — opens the default browser on a web search
  for the query (v1; a spoken summary is a possible upgrade — see
  `DECISIONS.md` #10)
...
```
(`apps.json` at the repo root: friendly name -> Windows `start` target.)
